...
async def create_chat(
    service: ChatServiceDep, data: ChatCreateSchema, owner_id: UserIdDep,
) -> ChatSchema:
...
async def get_all_chats(
    service: ChatServiceDep, user_id: UserIdDep,
) -> Sequence[ChatSchema]:
...
async def create_private_chat(
    service: ChatServiceDep, user_id_1: UUID, users_id_2: UUID,
) -> ChatSchema:
